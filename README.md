...
After installation, run `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm install` and then run `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm  start` which will open up a preview of the template in your default browser, watch for changes to core template files, and live reload the browser when changes are saved. You can view the `gulpfile.js` to see which tasks are included with the dev environment.

#### Gulp Tasks

- `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm gulp` the default task that builds everything
- `docker run --rm -it -w /build -v "$PWD:/build:delegated" -p 3000:3000 node:dubnium npm gulp watch` browserSync opens the project in your default browser and live reloads when changes are made
- `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm gulp css` compiles SCSS files into CSS and minifies the compiled CSS
- `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm gulp js` minifies the themes JS file
- `docker run --rm -it -w /build -v "$PWD:/build:delegated" node:dubnium npm gulp vendor` copies dependencies from node_modules to the vendor directory
